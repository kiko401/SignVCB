...
echo "[1/15] GET /health"
curl -fsS "$BASE/health" | python -m json.tool
echo ""

echo "[2/15] GET /api/v1/app_config (匿名)"
curl -fsS "$BASE/api/v1/app_config" | python -m json.tool
echo ""

echo "[3/15] POST /api/v1/auth/register"
...
echo "[6/15] POST /api/v1/rewrite (SSE)"
...
echo "[8/15] POST /api/v1/normalize"
...
echo "[9/15] POST /api/v1/log_mismatch"
...
echo "[10/15] GET /api/v1/reading/books"
curl -fsS "$BASE/api/v1/reading/books" \
  -H "Authorization: Bearer $TOKEN" | python -m json.tool
echo ""

echo "[11/15] GET /api/v1/reading/content?book_id=1"
curl -fsS "$BASE/api/v1/reading/content?book_id=1" \
  -H "Authorization: Bearer $TOKEN" | python -m json.tool
echo ""

echo "[12/15] GET /api/v1/practice/question?level=L1&type=word_match"
curl -fsS "$BASE/api/v1/practice/question?level=L1&type=word_match" \
  -H "Authorization: Bearer $TOKEN" | python -m json.tool
echo ""

echo "[13/15] POST /api/v1/practice/validate"
